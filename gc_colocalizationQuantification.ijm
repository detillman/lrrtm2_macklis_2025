var LUTs=newArray("Green", "Red", "Magenta");
var nLabels=2;
var in="";
var out="";
var labels=newArray();
...
function GUI(nLabels, isSingle){
	...
	Dialog.addNumber("Diameter_of_the_detection_square", 41);
	Dialog.addNumber("Radius_for_spots_filtering", 2);
	Dialog.addNumber("Noise_tolerance_for_spots_detection", 3);
	Dialog.addNumber("Min_size_for_spots_(pixels)", 2);
	Dialog.addNumber("Max_size_for_spots_(pixels)", 40);
	Dialog.addNumber("Diameter_of_the_quantification_circle_square", 20);
	Dialog.addNumber("Pixel_size_in_microns", 0.162);
	Dialog.addMessage("---Randomization---");
	Dialog.addNumber("#_monte_carlo_simulations", 1000);
	Dialog.addNumber("distance_max_colocalization_(microns)", 0.45);
	Dialog.show();
...
function overlayAndCutOut(labels, nLabels){
	...
	for(i=0; i<nLabels; i++) {
		arg+="c"+(i+1)+"=["+labels[2*i]+"] ";
	}
	
	//for(i=0; i<nLabels; i++) {
		//if(LUTs[i]=="Red"){
			//arg+="c1=["+labels[2*i]+"] ";
		//} else if (LUTs[i]=="Green") {
			//arg+="c2=["+labels[2*i]+"] ";
		//} else if (LUTs[i]=="Blue") {
			//arg+="c3=["+labels[2*i]+"] ";
		//} else if (LUTs[i]=="Gray") {
		...
		//} else if (LUTs[i]=="Cyan") {
			//arg+="c5=["+labels[2*i]+"] ";
		//} else if (LUTs[i]=="Magenta") {
			//arg+="c6=["+labels[2*i]+"] ";
		//} else if (LUTs[i]=="Yellow") {
			//arg+="c7=["+labels[2*i]+"] ";
		//}
	//}
	...
	//run("Merge Channels...", arg+"create keep ignore");
	rename("Composite");
...
function reviewSynaptosomes(size, dimensions){
	...
	while (!isKeyDown("shift")){
		showStatus("Click on the synaptosome to change its status, then press shift");
